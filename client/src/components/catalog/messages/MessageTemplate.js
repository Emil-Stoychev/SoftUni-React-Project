...
export const MessageTemplate = ({ data, cookie, setMessages, navigate, setCookies, setErrors, errors }) => {
    ...
    let word =
        data.title.includes('deleted')
            ? 'DELETED PRODUCT'
            : data.title.includes('edited')
                ? 'EDITED PRODUCT'
                : data.title.includes('created')
                    ? 'CREATED PRODUCT'
                    : data.title.includes('have purchased')
                        ? 'PURCHASED NEW PRODUCT'
                        : data.title.includes('was purchased')
                            ? 'PRODUCT SOLD'
                            : data.title.includes('surprise from Daily Wheel')
                                ? "DAILY WHEEL"
                                : ''
